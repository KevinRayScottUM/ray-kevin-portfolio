...
async function ensurePostRow(db, postId) {
  ...
  await db
    .prepare("INSERT INTO blog_likes (post_id, likes) VALUES (?1, ?2) ON CONFLICT(post_id) DO NOTHING")
...
export async function onRequestPost(context) {
  ...
  await ensurePostRow(db, postId);
  await db
    .prepare("UPDATE blog_likes SET likes = likes + 1, updated_at = CURRENT_TIMESTAMP WHERE post_id = ?1")
    .bind(postId)
    .run();
